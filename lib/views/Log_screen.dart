            ...
            child: GetBuilder<LogController>(builder: (controller){
              return ListView.builder(itemCount: logController.productData.length, itemBuilder: (context, index) {
                return Container(padding: EdgeInsets.all(5),
                    ...
                    child: InkWell(
                      onTap: (){
                        if(logController.productData[index].request == 'Request Accepted'){
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>RequestAccepted()));
                        }
                        else if(logController.productData[index].request == 'Request Rejected'){
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>RequestRejected()));
                        }
                        else{
                          Navigator.push(context, MaterialPageRoute(builder: (context)=>RequestPending()));
                        }
                      },
                      //Dispaly of Requests
                      child: Card(
